...
| `studio-dashboard.jpg` | `/studio/dashboard` | `1280x1024@2` | Captured after dashboard metric/activity polish; remaining shell width/avatar/spacing drift |
...
| `fan-dashboard.png` | `/hub/dashboard` | `1280x1024@2` | Captured after Fan Hub local-card imagery polish; remaining P2 is overall density/spacing rather than missing/remote card art |
...
| `super-admin.png` | `/admin/dashboard` | `1280x1024@2` | Captured after Super Admin proportion-stack polish; row-count/data-density/action-icon/filter/threshold-card/stack drift reduced, remaining P2 page-height drift |
...
- [P2] Several screens use the right brand direction but still need final copy/data-density polish
  ...
  Evidence: teal/navy palette, rounded cards, soft shadows, and LinkShelf branding are present. Super Admin now uses the Stitch `Global Revenue Ledger` first screen, full LinkShelf Admin rail, four platform metric cards, Traffic Split Monitor, Global Thresholds, Withdrawal Approval Pool, and Active Creator Directory. The latest `test-results/design-qa/latest/compare/super-admin.png` recapture shows the admin tables now match the Stitch row count/density story with `4 Pending`, two withdrawal rows, two creator rows, the Stitch-like `visibility` + `check_circle` withdrawal action icons, the compact `filter_alt` creator-directory control, and the Stitch-like default Global Thresholds card with `Apply Global Rules`. The app keeps rejection available through a details dialog instead of exposing a main-row cancel icon, creator filtering remains available through a click-to-open input, and minimum-withdrawal editing remains available through a threshold rules dialog. The latest pass also matches the Stitch column-stack structure by using flex column stacks inside the lower admin grid, removes the right-side table-card stretch blank space, limits the default Traffic Split log to the three visible Stitch rows, and tightens metric-card minimum height. The Super Admin capture height dropped from 2378px to 2210px after threshold-card polish and then to 2162px after the stack/log-density pass; the reference is still 2048px tall. Remaining visible drift is now narrower: the overall first screen is still slightly taller than the Stitch source. Landing and creator profile still differ in text, metric values, card density, supporting sections, avatar/photo choices, and per-screen navigation across the side-by-side comparisons.
  Impact: the app feels coherent, but not yet like a faithful Stitch export implementation.
  Fix: finish the remaining Super Admin page-height polish, then do landing/profile copy, mock data, card density, image usage, and spacing per screen.
...
Known blocker: current-run QA still has actionable P2 polish drift in share-modal source-state alignment, Studio visual proportions, Fan Hub density/module proportions, Super Admin page-height/proportion polish, and landing/profile data-density fidelity.
