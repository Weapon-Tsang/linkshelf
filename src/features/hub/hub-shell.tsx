  ...
  return (
    <div className="min-h-screen bg-[var(--surface-low)]/70 text-[var(--ink)] lg:grid lg:grid-cols-[256px_minmax(0,1fr)]">
      ...
      >
        ...
        <nav aria-label="Fan Hub" className="mt-11 grid grid-cols-2 gap-2 lg:grid-cols-1">
          {navItems.map((item) => {
            const active =
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex items-center gap-4 border-r-4 border-transparent px-4 py-4 text-base font-bold text-[var(--muted)] transition-colors hover:bg-[var(--surface-low)] hover:text-[var(--ink)] lg:px-4",
                  active &&
        ...
        <div className="mt-auto pt-8">
          <Link
            className="flex w-full items-center justify-center gap-3 rounded-2xl bg-[var(--teal-700)] px-5 py-5 text-lg font-bold text-white shadow-[0_14px_30px_rgba(0,191,174,0.22)] transition-transform hover:scale-[1.02] active:scale-95"
      ...
      <main className="min-w-0 px-5 py-8 sm:px-8 lg:px-10 lg:py-8">{children}</main>
