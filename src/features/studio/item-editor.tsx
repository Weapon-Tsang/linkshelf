...
export function ItemEditor({
  ...
  const cardClass = compact
    ? "rounded-2xl border border-[var(--line)] bg-white/92 p-4 shadow-[0_10px_28px_rgba(11,19,43,0.04)]"
  ...
  const inputClass = compact
    ? "min-h-10 w-full rounded-xl border border-[var(--line)] bg-[var(--surface-low)] px-3 text-xs font-semibold outline-none focus:border-[var(--teal-700)]"
  ...
  const urlInputClass = compact
    ? "min-h-10 min-w-0 flex-1 rounded-xl border border-[var(--line)] bg-[var(--surface-low)] px-3 text-xs font-semibold outline-none focus:border-[var(--teal-700)]"
  ...
  return (
    ...
    >
      <div className={headerClass}>
        <legend className={compact ? "text-sm font-black" : "text-lg font-bold"}>
          Product {index + 1}
          {product.title ? (
            <span
              className={
                compact
                  ? "ml-2 text-xs font-semibold text-[var(--muted)]"
                  : "ml-2 text-sm font-semibold text-[var(--muted)]"
              }
            >
              {product.title}
            </span>
